...
void MediaHandler::SelectOutputPath()
{
    ...
    f.lpstrFilter = "MP4\0*.mp4\0GIF\0*.gif\0WEBM\0*.webm\0MOV\0*.mov\0WMV\0*.wmv\0AVI\0*.avi\0FLV\0*.flv\0APNG\0*.apng\0MPG\0*.mpg\0All Files\0*.*\0\0";
...
void MediaHandler::validateOutputFormat()
{
    const std::vector<std::string> SUPPORT_EXTS = {
        ".mp4", ".mov", ".wmv", ".gif", ".webm", ".avi", ".flv", ".apng", ".mpg"
    };
...
bool MediaHandler::StartRecord()
{
    ...
    else if(_ocodecCtx->codec_id == AV_CODEC_ID_APNG)
        _ocodecCtx->pix_fmt = AV_PIX_FMT_RGBA;
    else
